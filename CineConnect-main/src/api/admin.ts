...
function mapBackendAdminUser(u: BackendAdminUser): User {
  return {
    ...
    email: u.email,
    role: u.role,
    // GET /api/admin/users doesn't join user_roles (it's a listing endpoint,
    // not the authenticated user's own profile) — roles isn't needed there,
    // just satisfying the User type.
    roles: [u.role],
