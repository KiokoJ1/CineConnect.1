...
// A decision is "final" once the producer has explicitly accepted or
// rejected the application. Once final, the status can only change again
// if the caller passes `force: true` — i.e. the producer used the UI's
// explicit "Edit decision" action rather than double-tapping Accept/Reject.
const FINAL_STATUSES = ['shortlisted', 'declined'];

async function updateApplicationStatus(user, applicationId, status, force = false) {
  ...
  const alreadyFinal = FINAL_STATUSES.includes(application.status);
  const isNoOp = application.status === normalizedStatus;

  if (alreadyFinal && !isNoOp && !force) {
    throw createServiceError(
      409,
      `This application was already ${application.status === 'shortlisted' ? 'accepted' : 'rejected'}. ` +
        'Explicitly edit the decision to change it.',
